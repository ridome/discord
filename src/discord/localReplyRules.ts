function withGeminiDegradedPrefix(reply: string, geminiErrorMessage?: string | null): string {
  if (!geminiErrorMessage) {
    return reply;
  }
  return `Gemini 当前不稳定，我先本地回答：${reply}`;
}
...
export function buildContextualFallbackReply(
  ...
  if (/(读取|读文件|查看文件|read file|cat)/i.test(text) && /[a-zA-Z]:\\/.test(text)) {
    return withGeminiDegradedPrefix(
      "可以直接读本地文件。示例：`/read path:D:\\work\\sample-repo\\README.md`",
      geminiErrorMessage
    );
  }
  ...
  if (/(价格|费用|收费|成本|预算|贵|便宜|计费|token 用量|token)/i.test(text)) {
    return withGeminiDegradedPrefix(
      "如果主要担心价格，建议用分层策略：日常问答优先轻量模型，只有复杂改代码任务再切高性能模型。你要的话我可以按“省钱优先”给你一套默认路由。",
      geminiErrorMessage
    );
  }
  ...
  if (/^(你好|嗨|hi|hello)/i.test(text)) {
    return withGeminiDegradedPrefix(
      "在。你可以直接说需求，我能聊天，也能把可执行操作转成待确认命令。",
      geminiErrorMessage
    );
  }
  ...
  if (geminiErrorMessage) {
    return "Gemini 当前不稳定，我先本地处理。你继续说具体目标，我会给出可执行确认。";
  }
...
export function tryLocalIntentReply(userInput: string, defaultRepoId: string | null): string | null {
  ...
  if (/(价格|费用|收费|成本|预算|贵|便宜|计费|token 用量|token)/i.test(text)) {
    return "如果你主要担心价格，我建议默认走轻量模型，复杂任务再切到高性能模型。我可以帮你把这条策略设成默认。";
  }
